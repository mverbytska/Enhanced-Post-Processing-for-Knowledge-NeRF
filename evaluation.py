...
def compare_images(orig_img, enhanced_img, save_path=None):
    _, axes = plt.subplots(1, 2, figsize=(10, 5))
    axes[0].imshow(orig_img.cpu().detach().numpy().transpose(1, 2, 0))
    axes[0].set_title('Original Image')
    axes[0].axis('off')

    axes[1].imshow(enhanced_img.cpu().detach().numpy().transpose(1, 2, 0))
